...
- **Workflow**: 
  1. Modify code.
  2. **Versioning**: MUST update `APP_NAME` in `config.py` (only when `mwm-app` is modified).
...
## 3. Versioning Policy
- **Before Building**: Always update the `APP_NAME` in `config.py` with the current date and sequence number (only when `mwm-app` is modified).
...
- **Rule**: 사용자에게 설명할 때는 말을 짧고 명료하게 한다. 
