...
## 1. 개요
에이전트(Agent)에서 수행하는 새로운 `LogExtractor` (로그 추출) 기능을 서버(`mw_app`)에서 관리하고 전송할 수 있도록 `CommandClassEnum`을 확장하고, 관련 데이터베이스를 업데이트하는 방법입니다. 결과에 대한 자동실행(Autorun)은 생략된 기본 연동 과정만 설명합니다.
...
`migrations/versions/a1b2c3d4e5f6_.py` 파일로 `ALTER TYPE commandclassenum ADD VALUE IF NOT EXISTS 'LogExtractor'` 명령이 생성되었습니다. 서버 환경에서 아래 명령어를 실행하여 적용할 수 있습니다.
...
```sql
ALTER TYPE commandclassenum ADD VALUE IF NOT EXISTS 'LogExtractor';
```

## 4. UI에서 새 Command Type 템플릿 생성
코드 상에 `LogExtractor`가 반영되었으므로, 구동 후 브라우저 UI에서 다음을 수행합니다.
